...
class MetaConnector:
    ...
    def fetch_insights(
        ...
        # Build query parameters for the Insights API.  The Marketing API expects
        # `time_range` to be passed as a JSON‑encoded object (e.g.
        # time_range={"since":"2024-01-01","until":"2024-01-31"}).  Passing a
        # nested Python dict directly to `requests.get` can lead to the keys of
        # the dict (`since` and `until`) being used as *values* (for example
        # `time_range=since&time_range=until`) which causes a 400 error.
        import json
        params = {
            "level": "account",
            # encode the time_range as a JSON string per API requirements
